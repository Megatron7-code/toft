...
class Database(object):
    ...
    def init_session(self):
        for db, value in MYSQL.items():
            self.session['s'][db] = []

            master = value.get('master')
            session = self._session(master['user'], master['pass'],
                                    master['host'], master['port'], db)
            self.session['m'][db] = session
            slaves = value.get('slaves')

            for slave in slaves:
                session = self._session(slave['user'], slave['pass'],
                                        slave['host'], slave['port'], db,
                                        master=False)
    ...
    def close(self):
        ...
        def shut(ins):
            try:
                ...
                logging.error('MySQL server has gone away. ignore.%s' % e)
            finally:
                ins.close()
